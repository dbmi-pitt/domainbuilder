...
public class ShapeEntry implements Serializable, Comparable {
	 ...
	 * Create a tutor marker from ShapeEntry
	 */
	public Annotation getAnnotation(AnnotationManager mm){
		if(marker == null){
			...
			marker = mm.createAnnotation(name,shapeType,r,getColor(),false);
			marker.setTags(tags);
	...
	*/
	 ...
	 */
	public ShapeEntry clone(){
		ShapeEntry e = new ShapeEntry();
		e.setProperties(getProperties());
		return e;
	}
}
